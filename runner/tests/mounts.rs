...
use runner::store::{MountTable, Sink, Store};
...
fn a_path_renders_non_text_segments_escaped() {
    assert_eq!(runner::store::render(&path(&[b"iso", b"log"])), "/iso/log");
    assert_eq!(
        runner::store::render(&[b"iso".to_vec(), vec![0x00, 0xff]]),
        "/iso/\\x00\\xff"
    );
}
...
/// vocabulary, and kisal maps it to different errnos.
#[test]
