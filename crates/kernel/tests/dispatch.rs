...
use kernel::machine::{Machine, Registers};
use kernel::paths;
use kernel::syscall::{Arguments, Fault, Kernel, Outcome, number};
...
fn empty_image() -> kernel::image::Image<'static> {
    let baked: &'static image::Image = Box::leak(Box::new(image::bake_empty()));
    kernel::image::Image::parse(&baked.index, &baked.blob).expect("parse an empty image")
}
...
fn a_failing_store_becomes_an_errno() {
    ...
    assert_eq!(
        logged,
        "kernel: the store at /iso/console/stdout failed: the sink refused it"
    );
}
...
fn an_unimplemented_syscall_faults_and_names_itself() {
    ...
    assert_eq!(
        message,
        "kernel: unimplemented syscall 175 with (0, 0, 0, 0, 0, 0)"
    );
}
...
fn an_unknown_number_is_reported_as_a_number() {
    ...
    assert_eq!(
        message,
        "kernel: unimplemented syscall 9999 with (0, 0, 0, 0, 0, 0)"
    );
}
...
/// A sub-function Linux *does* have and the kernel does not implement is a named
...
use kernel::abi::{ReadResult, Slice, WriteResult};
...
fn the_option_discriminant_follows_declaration_order() {
    assert_eq!(kernel::abi::OPTION_NONE, 0);
    assert_eq!(kernel::abi::OPTION_SOME, 1);
}
...
fn the_two_arms_of_a_read_result_do_not_leak_into_each_other() {
    let ok_some = ReadResult {
        discriminant: 0,
        arm: [kernel::abi::OPTION_SOME, 0x1000, 5],
    };
    ...
    let ok_none = ReadResult {
        discriminant: 0,
        arm: [kernel::abi::OPTION_NONE, 0, 0],
    };
