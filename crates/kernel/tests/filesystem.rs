...
use kernel::machine::Registers;
use kernel::syscall::{Arguments, Kernel, Outcome, number};
...
fn tree(label: &str) -> Tree {
    let unique = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("the kernel-fs-{label}-{unique}"));
...
fn mounted_tree(label: &str) -> Tree {
    let unique = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("the kernel-mnt-{label}-{unique}"));
...
impl Store for ConsoleStore {
    fn read(&mut self, path: &[&[u8]], into: &mut Vec<u8>) -> StoreOutcome {
        if path == kernel::paths::CONSOLE_STDIN && !self.input.is_empty() {
...
impl Arena {
    ...
    fn new() -> Self {
        Self {
            region: cpu::arena::Arena::new(Self::LENGTH as u64),
            used: 0,
        }
    }
...
fn fcntl_reads_and_writes_the_flags_it_owns() {
    ...
    assert_eq!(
        ...
        (open_flags::READ_ONLY | kernel::file::O_LARGEFILE) as i64
    );
    assert_eq!(
        fixture.call(number::FCNTL, [fd, fcntl_command::GETFD as i64, 0, 0, 0, 0]),
        0
    ...
    assert_eq!(
        fixture.call(number::FCNTL, [fd, fcntl_command::GETFL as i64, 0, 0, 0, 0]),
        (open_flags::READ_ONLY | open_flags::NONBLOCK | kernel::file::O_LARGEFILE) as i64,
...
fn getdents64_reports_entries_in_order_with_their_type() {
    ...
    use kernel::image::directory_entry_type as kind;
    assert_eq!(
        entries,
        vec![
...
fn write_follows_the_descriptor_rather_than_its_number() {
    ...
    assert_eq!(
        fixture.kernel.store.contents(kernel::paths::CONSOLE_STDOUT),
        b"to stdout"
    );
...
fn the_attribute_list_is_names_one_after_another() {
    ...
    // "user.empty\0user.origin\0" — sorted, because the packager sorts them.
    let total = 11 + 12;
    assert_eq!(
...
fn the_mount_table_refuses_what_it_cannot_represent() {
    ...
    while attached < kernel::mount::MAX_MOUNTS {
    ...
    let last = kernel
        .vfs
        .mounts()
        .root_of(kernel::mount::MAX_MOUNTS as u8 - 1)
        .expect("root");
    assert_eq!(
...
/// what the kernel reads, the patch writes it with the same `Inode::write` the
/// packager uses, and without it the whole device branch — `of_mode`'s
...
fn a_character_device_in_the_image_reports_as_one() {
    ...
    for (major, minor) in [(5u32, 1u32), (8, 300)] {
        ...
        let mut baked = image::bake_directory(&tree.root).expect("bake");
        patch_inode_to_device(&mut baked, "pipe", major, minor);

        let baked: &'static image::Image = Box::leak(Box::new(baked));
        let image = kernel::image::Image::parse(&baked.index, &baked.blob).expect("parse");
        let arena = Arena::new();
        let mut fixture = Fixture {
            kernel: Kernel::new(
...
/// kernel reads, using the same writer the packager uses.
fn patch_inode_to_device(baked: &mut image::Image, name: &str, major: u32, minor: u32) {
    let (number, position) = {
        let image = kernel::image::Image::parse(&baked.index, &baked.blob).expect("parse");
        ...
        (
            entry.inode,
            etc.payload as usize + 4 + position as usize * kernel::image::DIRENT_SIZE,
        )
    };

    let inode_offset =
        u32::from_le_bytes(baked.index[12..16].try_into().expect("four bytes")) as usize;
    let at = inode_offset + number as usize * kernel::image::INODE_SIZE;
    ...
    let mut record = [0u8; kernel::image::INODE_SIZE];
    inode.write(&mut record);
    baked.index[at..at + kernel::image::INODE_SIZE].copy_from_slice(&record);

    // The dirent carries a `d_type` precomputed at bake time, so it has to
    // agree — an image where the two disagree is one the packager cannot
    ...
    baked.index[dirent_offset + position + 8] =
        kernel::image::directory_entry_type::of_mode(inode.mode);
}
...
///
/// Unreachable from a bake — the packager writes both from the same walk — so
...
fn a_directory_whose_parent_disowns_it_is_refused() {
    ...
    inode.parent = kernel::image::Image::parse(&baked.index, &baked.blob)
        .expect("parse")
        .root();
    let mut record = [0u8; kernel::image::INODE_SIZE];
    inode.write(&mut record);
...
fn compressed_contents_round_trip_and_a_wrong_length_is_loud() {
    use kernel::image::{Image, INODE_SIZE, inode_flags};
    ...
    let baked = image::bake_directory(&tree.root).expect("bake");
