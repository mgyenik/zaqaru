//! The syscall dispatcher: one table, one row per Linux syscall the kernel
...
/// The Linux syscall numbers the kernel knows about. Named as constants rather
...
fn flatten(pages: &mut cpu::space::Space, enforcement: Enforcement, from: u64) {
    ...
    let first = from.max(cpu::space::PAGE_SIZE).min(limit);
    pages.protect(first, limit - first, cpu::space::Protection::ALL);
}
...
impl Fault {
    ...
    pub fn message(&self, into: &mut String) {
        into.push_str("kernel: unimplemented syscall ");
        match self.name {
            Some(name) => {
                into.push_str(name);
                into.push_str(" (");
...
impl<'a, S: Store, M: Machine> Kernel<'a, S, M> {
    ...
    fn reserve_space(machine: &mut M) -> (crate::space::Space, cpu::space::Space) {
        // The guest's arenas are a block reserved once, here, and not
        // whatever happens to lie above the module. They have to be: the
        // kernel's own allocator takes its pages from the end of linear
        // memory, and so would the arenas — two claimants on the same
        ...
        (space, cpu::space::Space::new(machine.memory_limit()))
    }
    ...
    fn mount_synthetics(&mut self) {
        for (path, index) in [
            ...
            if let Err(errno) = self.vfs.mounts_mut().replace(point, image) {
                ...
                // program's path.
                let mut message = String::from("kernel: cannot mount ");
    ...
    fn verify_pages(&self) {
        use crate::space::prot;
        for vma in self.space.vmas() {
            let wanted = cpu::space::Protection {
                read: vma.prot & prot::READ != 0,
                write: vma.prot & prot::WRITE != 0,
                execute: vma.prot & prot::EXEC != 0,
            };
    ...
    /// syscall the kernel implements it implements at its modern shape — there
    ...
    pub(crate) fn report_store_failure(&mut self, path: &[&[u8]]) {
        ...
        message.extend_from_slice(b"kernel: the store at ");
        for segment in path {
            message.push(b'/');
