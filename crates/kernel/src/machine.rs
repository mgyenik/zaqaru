...
pub trait Machine {
    ...
    fn deactivate(&mut self, pages: &cpu::space::Space) {
        let _ = pages;
    }
...
impl Default for Registers {
    fn default() -> Self {
        Self {
            segment_base: 0,
            stack_pointer: 0,
            memory_limit: cpu::space::CEILING,
            ceiling: cpu::space::CEILING,
...
///
/// Inside the module this is `image::layout`'s job: a container carrying a
...
impl Interpreted {
    ...
    pub fn thread(&self) -> &cpu::state::Tcb {
        &self.threads.current().tcb
    }

    pub fn thread_mut(&mut self) -> &mut cpu::state::Tcb {
    ...
    pub fn new() -> Self {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        assert!(
            cpu::arena::commit(cpu::arena::FLOOR + PROGRAM_REGION),
...
impl<const N: usize> GuestBytes<N> {
    pub fn new() -> Self {
        let arena = cpu::arena::Arena::new(N.max(1) as u64);
        Self { arena }
    }
...
impl GuestBuffer {
    ...
    pub fn of(bytes: &[u8]) -> Self {
        let arena = cpu::arena::Arena::new(bytes.len().max(1) as u64);
