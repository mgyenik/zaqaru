...
//! linear memory can do because linear memory is ours; `image::layout` is
...
pub enum Error {
    ...
    /// a bake that did not reserve the region — see `image::layout`.
...
impl Error {
    ...
    pub fn message(&self, into: &mut String) {
        into.push_str("kernel: cannot load the program: ");
        match self {
...
pub mod auxv {
    ...
    /// The vDSO's base. Supplied, since `kernel::vdso`: without it libc has
...
mod tests {
    ...
    fn every_refusal_says_what_it_was() {
        ...
        for error in [
            ...
            assert!(message.starts_with("kernel: cannot load the program: "));
...
///
/// `wasm-ld` defines this, and `image::layout` is what decides its value: a
