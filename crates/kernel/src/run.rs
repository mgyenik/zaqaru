...
use cpu::exec::Trap;
use cpu::state::Tcb;
use cpu::{Engine, Outcome as Step};
...
pub enum Exit {
    ...
    Signalled {
        ...
        /// not the same bug.
        access: Option<cpu::space::Access>,
    },
    ...
    Unsupported(cpu::exec::Unsupported),
...
impl<'a, S: Store> Process<'a, S> {
    ...
    ///
    /// Everything here is the kernel's existing `execve`: the segments are placed
    ...
    pub fn boot(
        kernel: Kernel<'a, S, Interpreted>,
        path: &[u8],
        argv: &[&[u8]],
        envp: &[&[u8]],
    ) -> Result<Self, crate::exec::Error> {
        Self::boot_with_cache(kernel, path, argv, envp, BlockCache::new())
    }

    /// As [`Process::boot`], with the block cache the process — and every
    /// process forked or exec'd from it — will decode into. The cache's
    /// policy is the one choice: [`BlockCache::new`] transpiles blocks to
    /// bytecode, [`BlockCache::interpreting`] never does.
    ...
    ///
    /// `cache` is what the new program decodes into: an empty one, since
    /// nothing decoded before it is worth anything now that the bytes at
    /// every address have changed.
    pub fn enter(
        mut kernel: Kernel<'a, S, Interpreted>,
        path: &[u8],
        argv: &[&[u8]],
            ...
            Trap::Fault(fault) => (
                ...
                Cause {
                    ...
                    code: match fault.access {
                        cpu::space::Access::Write => code::ACCERR,
                        _ => code::MAPERR,
                    },
    ...
    fn declined(&mut self, signal: i32, delivery: crate::syscall::Delivery) {
        ...
        let mut message = String::from("kernel: signal ");
