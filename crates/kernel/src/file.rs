...
impl<S: Store, M: Machine> Kernel<'_, S, M> {
    ...
    /// filesystem that lost them, so these rows exist as soon as the packager
    /// writes the region.
        ...
        let result = match command {
            ...
            | fcntl_command::OFD_SETLKW => {
                return Outcome::Fault(Fault::detailed(
                    number::FCNTL,
                    arguments,
                    "record locks, which the design places in the kernel as in-guest state",
                ));
            }
    ...
    pub(crate) fn statfs(&mut self, number: i64, arguments: Arguments) -> Outcome {
        ...
        let free = cpu::space::CEILING.saturating_sub(used);
