...
use cpu::space::{PAGE_SHIFT, PAGE_SIZE};
...
pub(crate) fn new_token() -> Token {
    RESIDENCY.with_borrow_mut(|held| {
        ...
        if held._arena.is_none() {
            held._arena = Some(cpu::arena::hold_addresses());
        }
        held.live.push(true);
...
/// same one every load and store in `cpu::space` uses. The caller must
...
mod tests {
    ...
    fn committed(pages_wanted: u64) -> u64 {
        let base = cpu::arena::FLOOR + (64 << 20);
        assert!(cpu::arena::commit(base + pages_wanted * PAGE_SIZE));
        base
    }
