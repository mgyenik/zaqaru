...
//! `the kernel/vdso/vdso.c` and its version script. It targets x86-64 whatever
...
fn main() {
    ...
    let built = Command::new(&compiler)
        ...
        .unwrap_or_else(|error| {
            panic!(
                "the kernel needs a C compiler to build its vDSO, and running \
                 `{compiler}` failed: {error}. Set `CC` to one, or see \
                 `the kernel/vdso/vdso.c` for what it is for."
            )
        });
    // The linker warns that the image has one RWX segment. It does, and it
    // does not matter: the kernel maps the vDSO read-and-execute whatever the
