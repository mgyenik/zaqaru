...
fn the_linker_keeps_the_blob_page_aligned() {
    ...
    let object = image::object::emit(&baked).expect("emit");
...
fn a_region_too_large_to_address_is_refused() {
    assert!(image::object::refuse_unaddressable("__image_blob", 0).is_ok());
    assert!(image::object::refuse_unaddressable("__image_blob", u32::MAX as usize).is_ok());
    let refusal = image::object::refuse_unaddressable("__image_blob", u32::MAX as usize + 1)
