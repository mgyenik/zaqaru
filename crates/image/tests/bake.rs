...
use kernel::image::{Image, directory_entry_type, file_type, inode_flags};
...
fn fixture(label: &str) -> Fixture {
    ...
    // An ELF-shaped file, which the packager page-aligns in the blob.
    write(
...
/// Walks a path through the index the way the kernel's resolution loop will:
/// component by component, asking the store one dumb question each time.
fn resolve<'a>(image: &Image<'a>, path: &str) -> Option<kernel::image::Inode> {
...
fn bake(fixture: &Fixture) -> image::Image {
    image::bake_directory(&fixture.root).expect("bake the fixture")
}
...
fn a_baked_tree_reads_back_as_the_tree_it_was() {
    ...
    let names = image::describe(&baked).expect("describe");
    assert_eq!(
        names,
        vec![
            "/bin",
...
fn a_symlink_keeps_its_target() {
    ...
    // Resolution is the kernel's job, not the store's: the index reports the
    // target and says nothing about where it points.
    assert!(image.contents(&link).is_err());
}

/// Metadata survives verbatim, including bits the kernel does not act on. The
...
/// reason `ping` can work without being setuid. If the kernel ever honours file
...
///
/// This measures the *index*, walked by this file's own helper — not the kernel's
/// resolution loop and not `stat`, which have their own measurement in
/// `the kernel/tests/filesystem.rs` against the real syscall path. The earlier
...
/// `the kernel/tests/filesystem.rs`, driven through the syscall rows that would
...
fn a_damaged_index_is_refused_by_name() {
    ...
    assert_eq!(
        ...
        Some(kernel::image::ImageError::BadMagic)
    );
    ...
    assert!(matches!(
        ...
        Some(kernel::image::ImageError::UnsupportedVersion(_))
    ));
...
fn a_path_without_attributes_is_not_an_error() {
    ...
    assert_eq!(
        image::xattr::read(&fixture.root.join("etc/alpha")).expect("read"),
        Vec::new()
    );
    // `/proc` answers `ENOTSUP`, which is the other way a path has none.
    assert_eq!(
        image::xattr::read(std::path::Path::new("/proc/self/status")).expect("read"),
        Vec::new()
    );
}
...
fn a_value_too_large_for_the_index_is_refused() {
    ...
    assert!(image::xattr::refuse_oversize(b"user.x", 0, path).is_ok());
    assert!(image::xattr::refuse_oversize(b"user.x", u16::MAX as usize, path).is_ok());
    let refusal = image::xattr::refuse_oversize(b"user.x", u16::MAX as usize + 1, path)
