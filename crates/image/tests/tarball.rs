...
fn workspace(label: &str) -> Workspace {
    let unique = std::time::SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let root = std::env::temp_dir().join(format!("packager-tar-{label}-{unique}"));
...
fn every_format_reads_back_as_the_tree_it_was() {
    for format in ["ustar", "gnu", "pax"] {
        ...
        let entries = image::tar::read(&bytes)
...
fn a_long_path_survives_in_every_format() {
    for format in ["ustar", "gnu", "pax"] {
        ...
        let bytes = archive(&workspace, &[&format!("--format={format}")]);
        let entries = image::tar::read(&bytes).expect("read");
        let relative = deep
            .strip_prefix(&tree)
...
fn a_damaged_header_is_refused() {
    ...
    flipped[10] ^= 0xff;
    let refusal = image::tar::read(&flipped).expect_err("a damaged header must be refused");
    assert!(
    ...
    let refusal = image::tar::read(&unknown).expect_err("an unknown record must be refused");
    ...
    let whole = image::tar::read(&bytes).expect("read");
    ...
    // are not there.
    let refusal = image::tar::read(&bytes[..bytes.len() / 2])
    ...
    let marker = bytes
        ...
        * 512;
    let refusal =
        image::tar::read(&bytes[..marker]).expect_err("an unmarked archive must be refused");
    assert!(
