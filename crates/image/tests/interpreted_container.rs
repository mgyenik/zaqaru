...
fn link_engine(workspace: &WorkingDirectory, image: &[u8], label: &str) -> std::path::PathBuf {
    ...
    let objects = vec![
        image,
        support::wasm_staticlib("zaqaru-kernel", "libkernel.a"),
        support::wasm_staticlib("zaqaru-cpu", "libcpu.a"),
        support::wasm_staticlib("zaqaru-x87", "libx87.a"),
    ];
...
fn module_for(
    ...
    let object = image::object::emit(&baked).expect("emit the image object");
