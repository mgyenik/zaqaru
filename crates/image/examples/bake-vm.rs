...
//!
//! The whole bake: a filesystem becomes an image, the image becomes one
//! object file, and the object is linked with three staticlibs that were
//! compiled once and are the same in every container. Nothing reads the
//! program.
//!
//! ```text
...
fn main() -> anyhow::Result<()> {
    ...
    let tree = match root.is_dir() {
        true => image::tree::Tree::from_directory(&root)?,
        false => image::layers::tree_from_archive(&std::fs::read(&root)?)?,
    };
    let baked = if root.is_dir() {
        ...
        let (image, invocation) = image::bake_archive_as_configured(&archive, &given)?;
        anyhow::ensure!(
    ...
    let object = image::object::emit(&baked)?;
    ...
    command
        ...
        .args(image::layout::link_arguments())
        .arg("-o")
        .arg(&output);
...
fn staticlib(crate_name: &str, library: &str) -> anyhow::Result<PathBuf> {
    // The workspace root: the crate is a member, and the staticlibs are
    // other members.
    let root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("the crate lives in the workspace");
    // A separate target directory per crate, so these builds never contend
    // with a native build in the workspace's own.
