//! This packager's flattening of a `docker save` archive, against `docker
...
//! `packager/tests/archive.rs`. This is the oracle for the real thing, and it is
...
//! cargo run -p packager --example image_differential -- saved.tar flat.tar
//! ```
//!
...
fn main() {
    ...
    let tree = image::layers::tree_from_archive(&std::fs::read(&saved).expect("read the archive"))
        .expect("flatten the layer stack");
    let mut ours = BTreeMap::new();
    walk(&tree, ROOT, "", &mut ours);

    let flat = image::tar::read(&std::fs::read(&exported).expect("read the export"))
    ...
    for entry in &flat {
        ...
        let shape = match entry.kind {
            image::tar::Kind::Directory => format!("dir {:o}", 0o040000 | (entry.mode & 0o7777)),
            image::tar::Kind::Regular => format!(
                "file {:o} {} bytes",
            ...
            image::tar::Kind::HardLink => "hardlink".to_string(),
            image::tar::Kind::Symlink => format!(
                "link {:o} -> {}",
                0o120000 | (entry.mode & 0o7777),
                String::from_utf8_lossy(&entry.link)
            ),
            image::tar::Kind::Fifo => format!("special {:o} 0", 0o010000 | (entry.mode & 0o7777)),
            image::tar::Kind::Character => "special-chr".to_string(),
            image::tar::Kind::Block => "special-blk".to_string(),
        };
