//! The packager: an image tree flattened into the two data segments a container
//! module carries.
//!
//! The output is a blob of file contents and a packed index over it, in the
//! format `kernel::image` defines — defined *there*, by the thing that reads
//! it, so that a writer with its own idea of the layout cannot exist.
//!
//! What the packager preserves is deliberately more than what the kernel honours.
...
use kernel::image::{self, HEADER_SIZE, INODE_SIZE, Inode, directory_entry_type, inode_flags};

/// Contents are packed at this alignment normally.
const CONTENT_ALIGNMENT: usize = 16;
/// Files the packager identifies as mapping candidates get page alignment
...
/// opaque-directory markers — reaches the same packager through the same tree;
...
/// list leaves the kernel at its default.
...
impl Builder {
    ...
    fn finish(mut self, root: u32) -> Result<Image> {
        ...
        for pending in &mut self.inodes {
            ...
            pending.inode.size =
                (DIRENT_COUNT_PREFIX + entries.len() * kernel::image::DIRENT_SIZE) as u64;
        ...
        let command_offset = xattr_offset + self.xattrs.len();
...
pub fn describe(image: &Image) -> Result<Vec<String>> {
    let parsed = kernel::image::Image::parse(&image.index, &image.blob)
