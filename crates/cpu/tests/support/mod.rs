...
fn mirror(tracee: &Tracee, registers: &libc::user_regs_struct) -> (Machine, Vec<Arena>) {
    ...
    let mut engine = Machine {
        tcb: Tcb::new(),
        space,
        // Interpreted, one instruction at a time. A bytecode trace retires a
        // whole block before it checks its budget, and the oracle compares
        // machines after every instruction; the transpiler has its own
        // tests against the interpreter.
        cache: BlockCache::interpreting(),
    };
