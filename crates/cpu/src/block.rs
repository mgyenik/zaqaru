...
pub struct Block {
    ...
    /// The bytecode trace transpiled from this block — an accelerator the
    /// run loop runs in place of interpreting, with anything unmodelled
    /// deferring back. `None` when the transpiler declined the block
    /// outright, or when the cache was told to interpret only.
...
pub struct BlockCache {
    ...
    pub decoded: u64,
    pub flushes: u64,
    /// Whether decoded blocks get a bytecode trace. Off, the run loop is
    /// the plain interpreter, one instruction at a time — which is what the
    /// lockstep oracle compares against hardware, and what a run that wants
    /// per-instruction preemption asks for.
    accelerate: bool,
}
...
impl BlockCache {
    /// A cache whose blocks are transpiled to bytecode: the default.
    pub fn new() -> Self {
        Self::with_acceleration(true)
    }

    /// A cache whose blocks are only ever interpreted.
    pub fn interpreting() -> Self {
        Self::with_acceleration(false)
    }

    fn with_acceleration(accelerate: bool) -> Self {
        Self {
            blocks: Vec::new(),
            free: Vec::new(),
    ...
    /// An empty cache with this one's policy, for a process whose bytes
    /// have all just changed.
    pub fn fresh(&self) -> Self {
        Self::with_acceleration(self.accelerate)
    }

    /// Whether this cache transpiles the blocks it decodes.
    pub fn accelerates(&self) -> bool {
        self.accelerate
    }
    ...
    pub fn entry(&mut self, address: u64, space: &mut Space) -> Result<usize, FetchError> {
        ...
        let block = decode(address, space, self.accelerate)?;
...
/// Attaches the bytecode trace to a freshly decoded block, when the cache
/// accelerates.
fn finalize(mut block: Block, accelerate: bool) -> Block {
    if accelerate {
...
fn decode(address: u64, space: &Space, accelerate: bool) -> Result<Block, FetchError> {
        ...
        if instruction.is_invalid() {
            return match decoder.last_error() {
                ...
                DecoderError::NoMoreBytes if !instructions.is_empty() => {
                    ...
                    return Ok(finalize(block, accelerate));
                }
