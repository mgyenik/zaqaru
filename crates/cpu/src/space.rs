...
impl Space {
    ...
    /// `image::layout` interleaves them — the program's segments low, the
    ...
    fn permitted(&self, address: u64, length: u64, access: Access) -> Result<(), Fault> {
        ...
        // the single-page test is exact, and on the last byte's address
        // existing at all: a range that wraps the address space is refused
        // by the checked arithmetic below, never waved through here.
        if length <= PAGE_SIZE
            && let Some(last_byte) = address.checked_add(length - 1)
        {
            let last = (last_byte >> PAGE_SHIFT) as usize;
