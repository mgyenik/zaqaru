...
//! [`targum::space::Space`]**, which is where the permission bits and the
...
use targum::space::{Access, Fault, Space, Unterminated};
...
impl<'a> GuestMemory<'a> {
    ...
    pub unsafe fn store_u64(&mut self, address: u64, value: u64) -> Result<(), Errno> {
        Ok(self
            .space
            .store(address, targum::state::Width::Qword, value)?)
    }
}
