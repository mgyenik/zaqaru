...
//! counter (`targum::exec::Cpu::timestamp`), so it plays the part of the
...
//!
//! `targum`'s comment on `rdtsc` used to say that nothing may calibrate
...
/// The compiled image, built by `build.rs` from `kisal/vdso/vdso.c`.
pub const IMAGE: &[u8] = include_bytes!(env!("KISAL_VDSO"));

/// What the kernel publishes for the vDSO to read.
///
/// **Must match `struct timebase` in `kisal/vdso/vdso.c` byte for byte**,
...
impl Timebase {
    ...
    pub fn assumed_rate() -> u64 {
        let per_instruction = targum::exec::TIMESTAMP_STEP;
...
const _: () = assert!(
    ...
    "the timebase's Rust layout must match the C one in `kisal/vdso/vdso.c`"
);
