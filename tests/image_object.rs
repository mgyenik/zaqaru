...
//!
//! Everything else about the baker is checked against the index it produces.
...
//! module are the bytes that were baked. Those are the properties kisal
...
fn the_linker_keeps_the_blob_page_aligned() {
    ...
    let object = baker::object::emit(&baked).expect("emit");
    ...
    // The image reads back through the same parser kisal uses, from the
    // bytes the linker placed rather than from the ones in this process.
    let image = kisal::image::Image::parse(&index, &blob).expect("parse");
...
/// a wasm data symbol carries, and the baker refuses to build one — but this
