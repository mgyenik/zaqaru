//! Shared plumbing for the packager's integration tests: scratch
//! directories, host tools (`wasm-ld`, `clang`, `cargo` for the wasm32
//! staticlibs), and the mount tables a container boots against.
...
/// Links relocatable wasm objects into a module.
...
/// Builds a workspace crate for wasm32, once per test process, and answers
/// its staticlib.
pub fn wasm_staticlib(crate_name: &'static str, library: &'static str) -> PathBuf {
    use std::sync::{Mutex, OnceLock};
    static BUILT: OnceLock<Mutex<std::collections::HashMap<&'static str, PathBuf>>> = OnceLock::new();
    ...
    // with the native one running this test.
    let target = root.join("target").join(format!("wasm-{crate_name}"));
    let output = Command::new(env!("CARGO"))
        .current_dir(root)
        .env("CARGO_TARGET_DIR", &target)
        .args(["build", "-p", crate_name, "--target", "wasm32-unknown-unknown", "--release"])
        .output()
...
pub fn console_mounts() -> runner::store::MountTable {
