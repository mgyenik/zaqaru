...
//! kisal finds them through link-time symbols rather than through addresses
...
/// The blob is page-aligned so that a file the baker marked `MMAP_ALIGNED`
...
///
/// The baker refuses to build an image with a region this large, so this is
...
/// is not one — and because kisal references the symbols unconditionally, so
